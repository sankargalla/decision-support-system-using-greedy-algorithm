from utils.main import quantile, conformal_set
from results.utils_real_data import split_dataset
from utils.main import user_accuracy
import numpy as np
...
if __name__ == "__main__":
    ...
    print("SE (with best alpha):", acc, std)
    ...
    best_SE_accuracy = (-np.inf, None, None)
    for best_alpha in deploy_avg_results_dict.keys():
        ...
        if acc > best_SE_accuracy[0]:
            best_SE_accuracy = (acc, deepcopy(prediction_sets), best_alpha, std)
        
    print("Best Alpha: ", best_SE_accuracy[2])
    print("SE:", best_SE_accuracy[0], best_SE_accuracy[3])
    prediction_sets = best_SE_accuracy[1]
